...
export const addCircleUser = ({ userID, circleID }) => {
  return dispatch => {
    const endpoint = '/api/circle/' + circleID + 'member';
    const parameters = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: userID }) };
    fetch(endpoint, parameters).then(response =>
      response.json().then(json => {
        if (response.status === 201) {
          console.log('success');
          dispatch(localAddCircleUser({userID: userID, circleID: circleID}));
        } else {
          console.log('failure');
        }
      })
