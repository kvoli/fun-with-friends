...
var addMember = async (req, res) => {
  try {
    ...
    await Circle.findOneAndUpdate({ _id: req.params.id }, { $addToSet: { members: addedMember } }, { useFindAndModify: false });
    if (isAdmin) {
      await Circle.findOneAndUpdate({ _id: req.params.id }, { $addToSet: { admins: addedMember } }, { useFindAndModify: false });
    }
    res.status(200).send();
  } catch (error) {
